...
def classify_ahead():
    ...
    if cond.state == "DISCONNECTED" or inval > 0.02:
        kind = "JUNCTION" if centre_m < 0.16 else "DISCONNECTED"
    else:
        kind = "NORMAL"
...
while True:
    ...
    if state == "SETTLE":
        ...
        elif d_cur is not None and inspected and s_ring >= d_cur["s"] - STOP_TOL:
            drive(0.0)
        ...
        if t_state > 0.7 * PHYSICS_HZ:
            ...
            _, pre_hole, pre_bead = inspect_defect(
        ...
        if t_state > 1.0 * PHYSICS_HZ:
            _, post_hole, post_bead = inspect_defect(
        ...
        if t_state == 1:
            cur += 1
            inspected, pre_hole, pre_bead = False, None, None
            nxt = (f"다음 결함 {cur + 1} (s "
        ...
        if s_hint <= START_S_MM * MM + 0.02:
            drive(0.0)
            end_reason = "관 단절 감지 후 진입점 복귀 완료"
