...
8. **가설 2(사용자 지적, 유력) — "로봇이 너무 길어서 관 굽는 지점에 걸쳐있다"**: `Body`↔`SphF`↔`DiscF` 기하를 계산하면 `DiscF`/`DiscR` 원점이 `Body`에서 각각 약 **±114mm**(총 ~228mm) 떨어져 있는데, 코스의 첫 직선(라이저) 구간은 겨우 **185mm**(굽음 시작 전까지). `Body`를 s=120mm에 놓으면 반대쪽 끝은 이미 굽은 구간(또는 그 너머)에 걸쳐 있을 수 있다 — 그런데 안착 진단은 **로봇 전체에 Body 기준 s 하나만** 써서 반경을 쟀으므로, 굽은 구간에 걸친 쪽은 애초에 잘못된 중심선 기준점으로 측정된 것일 수 있다. **즉 `DiscR` 이상은 실제 물리 문제가 아니라 진단 스크립트의 측정 오류일 가능성이 있다.**

## 지금 하고 있는 일 (미완료)

`check_rear_settle.py`를 고치는 중 — `real_map_demo.py::CenterLine.project()`(전체 중심선 탐색, 굽은 구간 포함)를 그대로 이식해서, **휠 12개 각각을 자기 위치에서 따로 투영**하도록 바꿨다(이전엔 Body 위치로 구한 s 하나를 전부에 재사용 — 이게 버그였을 가능성). 아직 재실행해서 결과 확인 전.

## 다음에 할 일

1. 고친 `check_rear_settle.py` 재실행 — 휠별 s(mm)와 반경(mm)을 같이 찍어서, `DiscR`이 실제로 굽은 구간에 들어가 있는지, 그 경우 반경이 정상으로 나오는지 확인.
2. 만약 가설 2가 맞으면: 이 로봇 길이(~228mm+)에는 `START_S_MM`을 어떻게 잡아도 어차피 한쪽 끝이 굽은 구간에 걸린다(185mm 직선 < 228mm 로봇) — 이건 "안착 실패"가 아니라 정상적인 상황일 수 있음. 그렇다면 다음 확인 포인트는 "굽은 구간에서도 3휠이 벽에 잘 붙어있는가"로 바뀌어야 함.
3. 안착 확인되면 `final_script.py`의 `go()`(실제 주행+분기조향)까지 실행해서 주행 자체가 되는지 확인 — 이건 아직 전혀 안 해봄.
4. `PIPE`/`JUNCTION` 상수 정리 — 지금 `check_rear_settle.py`는 `JUNCTION = Body 시작 위치`로 임시로 넣어놨다(진짜 분기점 아님, 안착 확인용 placeholder). floor2는 실제 T분기가 없어서(`real_map_demo.py` 기준 "disconnect"로 끝남) 분기조향 자체를 보려면 floor1 좌표(730,850mm 부근)가 필요할 수 있음.
