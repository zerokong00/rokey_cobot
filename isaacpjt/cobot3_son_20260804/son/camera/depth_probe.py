...
def build_scene(stage):
    ...
    ⚠ UsdGeom.Cylinder 를 쓰면 안 된다. 그것은 **뚜껑이 달린 속 찬 원기둥**
    이라 관 안에서 보면 화면 중앙에 끝면이 그대로 잡힌다. 빈 공간이 아니라
    0.215m 짜리 벽을 재게 되어 진단이 "판정불가" 로 끝난다(실측 확인).
    양끝이 열린 실제 배관 메시를 써야 그 너머가 진짜로 비어 있다.
    """
    UsdGeom.Xform.Define(stage, "/World")

    pts, idx = load_stl(SON / "pipe" / "meshes" / "pipe_straight.stl")
    # 600mm 직관의 앞끝을 HOLE_X 에 맞춘다 → 그 너머는 아무것도 없다
    xf = UsdGeom.Xform.Define(stage, "/World/Pipe")
    xf.AddTranslateOp().Set(Gf.Vec3d(HOLE_X - float(pts[:, 0].max()), 0.0, 0.0))
    mesh = UsdGeom.Mesh.Define(stage, "/World/Pipe/near")
